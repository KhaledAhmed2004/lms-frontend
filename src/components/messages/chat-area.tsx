...
export default function ChatArea({
  ...
  const [counterProposalMessageId, setCounterProposalMessageId] = useState<
    string | null
  >(null);
  const [selectedFiles, setSelectedFiles] = useState<File[]>([]);
  const [isFeedbackModalOpen, setIsFeedbackModalOpen] = useState(false);
  const [selectedSessionForFeedback, setSelectedSessionForFeedback] = useState<
    string | null
  >(null);
  const [isStudentReviewModalOpen, setIsStudentReviewModalOpen] =
    useState(false);
  ...
  const actualChatId = isSupportChatView
    ? adminChat?._id || ""
    : conversationId;
  ...
  const { mutate: proposeSession, isPending: isProposing } =
    useProposeSession();
  const { mutate: acceptProposal, isPending: isAccepting } =
    useAcceptSessionProposal();
  const { mutate: rejectProposal, isPending: isRejecting } =
    useRejectSessionProposal();
  const { mutate: counterPropose, isPending: isCounterProposing } =
    useCounterProposeSession();
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        ...
                        onClick={() =>
                          !(hasPendingProposal || hasActiveSession) &&
                          setIsScheduleOpen(true)
                        }
                        disabled={
                          isProposing || hasPendingProposal || hasActiveSession
                        }
                      >
                        {isProposing ? (
                          ...
                          <Calendar
                            className={`w-4 h-4 ${hasPendingProposal || hasActiveSession ? "text-gray-400" : "text-blue-500"}`}
                          />
                        )}
                      </Button>
                    </TooltipTrigger>
