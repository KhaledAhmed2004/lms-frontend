...
const STEPS = ["Application Review", "Interview", "Profile Setup"];

const computeProgressWidth = (step: number, screenWidth: number) => {
  if (screenWidth <= 640) {
    return step === 1 ? "15%" : step === 2 ? "55%" : step >= 3 ? "95%" : "0%";
  }
  if (screenWidth <= 768) {
    return step === 1 ? "12%" : step === 2 ? "52%" : step >= 3 ? "92%" : "0%";
  }
  return step === 1 ? "8%" : step === 2 ? "55%" : step >= 3 ? "95%" : "0%";
};

export const ApplicationProgressStepper = ({ step }: { step: number }) => {
