...
import { usePricingPlans, PricingPlan } from "@/hooks/api";

// Fallback pricing plans (shown while loading or if API fails)
const fallbackPricingPlans = [
  {
    name: "Flexible",
    tier: "FLEXIBLE" as const,
    pricePerHour: 30,
    courseDuration: "None",
    commitmentMonths: 0,
    minimumHours: 0,
...
export default function PricingSection() {
  ...
  const { data: pricingPlans, isLoading } = usePricingPlans();

  // Use API data if available, otherwise fallback
  const plans: PricingPlan[] = pricingPlans && pricingPlans.length > 0
    ? pricingPlans
              ...
              <div className="bg-white border border-gray-200 rounded-[28px] p-6 space-y-6">
                <div>
                  ...
                  <p className="text-3xl font-bold text-gray-900">
                    {plans[currentPlanIndex].pricePerHour}€
                  </p>
                </div>
                ...
                <div className="flex justify-between items-center">
                  ...
                  <p className="text-sm font-semibold text-gray-900">
                    {plans[currentPlanIndex].termType}
                  </p>
                </div>
                <hr className="border-t border-gray-200" />
                <div className="flex justify-between items-center">
                  ...
                  <div className="text-right">
                    <p className="text-xs font-semibold text-gray-900">
                      {plans[currentPlanIndex].selectedHours}
                    </p>
                    <p className="text-xs font-semibold text-gray-900 mt-1">
                      {plans[currentPlanIndex].selectedHoursDetails}
                    </p>
                  </div>
                </div>
                <hr className="border-t border-gray-200" />
                <div className="flex justify-between items-center">
                  ...
                  <p className="text-sm font-semibold text-gray-900">
                    {plans[currentPlanIndex].courseDuration}
                  </p>
                </div>
                <hr className="border-t border-gray-200" />
                <div className="flex justify-between text-right">
                  ...
                  <ul className="space-y-2">
                    {plans[currentPlanIndex].inclusions.map((inclusion, idx) => (
